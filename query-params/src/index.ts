...
app.get('/toys', async (c) => {
  ...
  // filter
  if (query.name) {
    data = data.filter((toy) => toy.name.toLocaleLowerCase().includes(query.name.toLocaleLowerCase()))
  }
  if (query.tag) {
    data = data.filter((toy) => toy.tags.some((tag) => query.tag.includes(tag))) //some ตรวจสอบเพียงบางอันต้องเป็นไปตาม Callback
  }
  ...
  //array.map คือการแปลงข้อมูลใน Array เป็นข้อมูลใหม่
  //.join() รวม Array ด้วยตัวแบ่งที่กำหนด
    return c.html(`
    <h1>Result</h1>
